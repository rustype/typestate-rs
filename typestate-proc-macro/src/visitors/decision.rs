...
use syn::{Error, Fields, Generics, Ident, ItemEnum, ItemMod, Variant, visit_mut::VisitMut};
...
impl<'sm> VisitMut for DecisionVisitor<'sm> {
    fn visit_item_enum_mut(&mut self, i: &mut ItemEnum) {
        ...
        for generic in self.decision_generics.clone() {
            for param in generic.params {
                i.generics.params.push(param);
            }
        }
